...
%% DVI, PS und PDF problemlos mï¿½glich!
...
%  twoside,             % Layout fuer beidseitigen Druck
	 oneside,						  % Layout fÃ¼r einseitigen Druck
   ...
   openany,             % Kapitel kÃ¶nnen auf geraden und ungeraden Seiten beginnen
...
%   draft               % Keine Bilder in der Anzeige, overfull hboxes werden angezeigt !Auskommentieren fÃ¼r Test-Compile!
]{scrbook}[2001/07/30]  % scrbook-Version mind. 2.8j von 2001/07/30

%% Pakete fÃ¼r nÃ¼tzliche Dinge
\usepackage{color} 							 % Schriftfarben verwenden
\usepackage{ngerman}             % neue deutsche Rechtschreibung
%\usepackage[ngerman]{translator} % Ãbersetzung 
...
\usepackage{ae}                  % FÃ¼r PDF-Erstellung
...
\usepackage{url}							 % URL in Literatur wird unterstï¿½tzt
...
\typearea[current]{current}        % Neuberechnung des Satzspiegels mit alten Werten nach Ãnderung von Zeilenabstand,etc

% -- Glossar --
\usepackage[toc, acronym]{glossaries} % GlossareintrÃ¤ge, muss nach hyperref (insofern dies verwendet wird) geladen werden
% (aufgrund der Seitenzahlverlinkung im Glossarverzeichnis), (benÃ¶tigt die Packete
...
\makeglossaries % erstellt ein Glossar (Verzeichnis fÃ¼r BegriffserklÃ¤rungen,
% z.B.: AbkÃ¼rzungen)
% GlossareintrÃ¤ge (MUSS fÃ¼r JEDEN Glossareintrag Ã¼berarbeitet werden):
\include{gloss}
...
\pagenumbering{Roman}           % Nummerierung RÃ¶misch Start bei I

\include{DeckHS}                % Deckblatt Nummerierung unterdrÃ¼ckt (In Deckblatt festgelegt)
%%\cleardoubleemptypage         % Die Eidesstattliche Erklaerung auf einer rechten Seite beginnen
\include{eidHS}                 % Eidesstattliche Erklaerung Nummerierung unterdrÃ¼ckt
...
\end{spacing}
%%% Vorbemerkungen %%%  Nummerierung unterdrÃ¼ckt durch *
...
\include{grund}                % Ein Kapitel des Hauptteils
\include{Problem}		% Ein weiteres Kapitel des Hauptteils
\include{EthischeBetrachtung}
\include{Anforderung}		% Die unterschiedlichen Kapitel
\include{Loesungsideen}
\include{BewertungLoesungsideen}
\include{LoesungVorschlag}		% des Hauptteils
\include{LoesungsAnforderung}		% sollten hier in der
\include{InfoEinePerson}		% Masterdatei einen sinnvollen
		% Kommentar erhalten
\include{InfoVielePerson}		% z.B. den Titel des Kapitels
\include{PhishingMail}		% um fÃ¼r Korrekturen oder Umstellungen
\include{Evaluation}		% leichter gefunden zu werden
...
%Abkï¿½rzungen ausgeben
\renewcommand{\acronymname}{AbkÃ¼rzungsverzeichnis}
...
%%% WICHTIG:
%% Um den Glossareintrag (AbkÃ¼rzungsverzeichnis richtig darstellen zu kÃ¶nnen, muss makeindex mit dem Parameter "-s Bachelorarbeit.ist -t Bachelorarbeit.alg -o
...
%% Unter Postprozessoren neuen Eintrag anlegen, z.B. Makeindex1. Unter Anwendung makeindex.exe auswÃ¤hlen. (c:\programme\miktex2.7\miktex\bin\makeindex.exe)
...
%% Analog zu oben im TeXnicCenter einen weiteren Eintrag Makeindex2 erstellen. In Linux einen weiteren makeindex-Aufruf im Script hinzufÃ¼gen.
...
%% Analog zu oben im TeXnicCenter einen weiteren Eintrag Makeindex3 erstellen. In Linux einen weiteren makeindex-Aufruf im Script hinzufÃ¼gen.
