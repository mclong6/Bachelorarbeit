...
Für die Personensuche im Internet wird die Google-Suchmaschine verwendet. Gesucht wird nach den eingegebenen Daten, welche über die Konsole eingelesen werden.
	...
	Zu Beginn werden alle Personen-Variablen mit einem leeren String initialisiert. Das bedeutet, alle Variablen, zu denen keine Information eingegeben wurde, enthalten einen leeren String.
		...
		Nachdem der Selenium WebDriver in das Scrapy Framework implementiert wurde, kann mit dem crawling begonnen werden. Der Web Crawler hat die Aufgabe den von Google vorgeschlagenen Webseiten zu folgen. Dazu muss zuerst die Webseite mit den Google-Suchergebnissen analysiert werden. Dadurch wird erkannt, wo sich die URLs für die vorgeschlagenen Seiten befinden. Sobald die Struktur der Seite bekannt ist, können die URLs gefunden, herausgelesen und gefolgt werden.
			...
			\end{figure}
			Im Ausschnitt des Seitenquelltextes \ref{img:GoogleSeitenquelltext} ist zu sehen, dass der <div>-Container mit der Klasse "'r"' zwei Hyperlinks enthält. URLs oder Links werden mit dem HTML-Tag <a> dargestellt. Allerdings wird nur einer der beiden vorkommenden Links benötigt. Die erste URL führt zu der gesuchten Webseiten. Die zweite führt ebenfalls zur gesuchten Webseite, jedoch wird diese übersetzt angezeigt. Infolgedessen wird nach dem erste Link gesucht.\\
			...
			Um zu erkennen, ob mehrere Seiten mit Suchergebnissen existieren, wird nach bestimmten Hyperlinks gesucht. Diese Links werden über das Attribute \textit{aria-label} identifiziert. Mit Hilfe der BeautifulSoup-Bibliothek und eines Regulären Ausdrucks, wird nach diesen Links gesucht. Die Reguläre Ausdruck, kurz Regex, beschreibt eine Zeichenkette, die aus dem Wort "'Page"' gefolgt von einer Nummer besteht. Ein Beispiel für die Übereinstimmung der Regex mit dem \textit{aria-label} wäre die Zeichenkette "'Page 3"'.
...
\section{Umsetzung: Herausfiltern von wichtigen Informationen auf einer Webseite}
	...
	Die korrekte Erstellung der Wortsammlungen ist sehr wichtig für das Projekt, da die Informationsgewinnung abhängig von diesen Listen ist. Das bedeutet, umso besser diese Datenbanken erstellt werden, umso mehr Informationen können gewonnen werden.
		...
		%TODO möglicherweise ausführlicher schreiben
